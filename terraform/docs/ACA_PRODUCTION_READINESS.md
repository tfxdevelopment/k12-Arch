...
- ✓ Container App and Environment diagnostic settings configured
- ✓ Dapr state store and pub/sub component support wired in Terraform
...
**Current State:** Key Vault integration is configured with managed identity-based secret reads.

**Remaining actions:**
- Ensure required secrets are present in `{env}apikv` before deployment.
- Keep connection strings out of `locals.tf`; provide them via secure pipeline/runtime variable injection (`TF_VAR_*`).
...
4. **Build pipeline:** Validate branch-based ACR build/deployment workflow in Azure DevOps
