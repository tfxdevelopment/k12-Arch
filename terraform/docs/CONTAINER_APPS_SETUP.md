...
### 0. Inject secure Terraform variables
Do not keep connection strings in `locals.tf`. Inject them at runtime:

```bash
export TF_VAR_sql_connection_string="<sql-connection-string>"
export TF_VAR_blob_storage_connection_string="<blob-connection-string>"
```
