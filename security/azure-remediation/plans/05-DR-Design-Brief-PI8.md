...
> SignalR are GA in Azure Government**, while **Static Web Apps remains not available in Gov in
> current public availability docs** (roadmap omission + Products-by-region table shows non-regional
> SWA only; accessed 2026-09-07). Gov-tenant state itself not yet queried.
...
Open items to confirm before 9/8 — status 2026-09-07: primary Gov region `[still open]`; ~~current replica type~~ **answered: geo-secondary (dev)**; ~~SWA / Front Door / SignalR availability~~ **answered: FD + SignalR GA in Gov; SWA not available in current public Gov availability docs (accessed 2026-09-07)**; APIM tier in prod `[still open — no APIM exists in commercial; check Gov]`; November add-on window for the prod drill `[still open]`.
