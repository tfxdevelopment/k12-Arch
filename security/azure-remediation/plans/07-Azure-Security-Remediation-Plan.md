# MyPortal K12 — Azure Security & Architecture Remediation Plan (DRAFT v0.3)
...
> **v0.3 (2026-09-13):** Gov preprod code review (ADO `k12-infra@main` + PR 6078, read-only) adds rows **G-DNS, G-VPN, G-PWD, G-TAG, G-URL**; **S2 corrected** — VMSS agent pools are Azure Public only, Gov agents = Container Apps Jobs; execution plan in `08-Gov-Preprod-Deploy-and-CICD-Plan.md`. Gov tenant queries still pending (Terry runs `../evidence/gov-preprod/discover-gov-preprod.sh` locally).
...
| S2 | Supply chain | ACI runners with PATs and mutable image tags (K12-8497). **Research: Managed DevOps Pools are commercial-cloud only — not available in Azure Government** | Target platform differs by tenant | Commercial: Managed DevOps Pool in VNet + WIF per K12-8497. **Gov: Container Apps Jobs (KEDA `azure-pipelines` scaler) or ACI self-hosted agents in the Gov VNet — VMSS agent pools are Azure Public only (corrected 2026-09-13); registration via a commercial-tenant service principal (Gov identities cannot join the commercial-tenant ADO org), scaler PAT read-only (plan 08 B2.3)**; digest-pinned images meanwhile | A/B | Yes (pilot) |
...
| G-DNS | Network | **New (code, 2026-09-13):** `modules/core/dns.tf` creates the private DNS zones with **commercial** names in every environment and every private endpoint binds to them (`api-enrollment/network.tf`, `messaging/main.tf`); Gov preprod cannot resolve `*.usgovcloudapi.net` / `azurecr.us` private-link names | Private endpoints ineffective in Gov; calls fail once public access is off | Cloud-aware zone map keyed by logical name + `moved` blocks (plan 08 B2.1); zone replacement in a window, resolution verified from inside the VNet | A | Preprod now; prod at build |
| G-VPN | Network | **New (code):** `modules/core/vpn.tf` P2S gateway hard-codes the commercial tenant/issuer/audience and is not gated by a variable | Gov gateway authenticates against the wrong cloud; cost of an unused gateway | `enable_vpn_gateway` + Gov Entra values (verify the Gov Azure VPN app ID) | A | Yes |
| G-PWD | Data | **New (code):** plaintext default credentials in `api-enrollment/variables.tf` (`sql_administrator_login_password`) and `core/variables.tf` (`vm_admin_password`) | Known admin password in git history | `random_password` → Key Vault secret; remove defaults; rotate the SQL admin password (apps use Entra auth) | A | Yes |
| G-TAG | Edge | **New (code):** App Service SCM allow-list uses commercial service tags `AzureCloud.<region>` and `AzureCloud.centralus` (`web-frontend/app.tf`) | Invalid or meaningless in Gov | Variable list of tags/IPs; Gov = agent-subnet NAT egress IP | A | Yes |
| G-URL | Logging | **New (code):** availability web test targets `<env>-api-enrollment.azure-api.net` (commercial APIM host that no longer exists) | Permanent false alerts | `availability_test_url` variable | A | Yes |
...
2. Gov availability — **ANSWERED** (evidence `01-azure-ado-research-notes.md`, sources accessed 2026-09-07, Copilot research PR #146 verified + integrated): Defender CSPM **GA in Gov**; **Defender for DevOps commercial-only** (no Gov connector); Front Door Standard/Premium **GA in Gov**; App Gateway WAF v2, SignalR, Service Bus Premium, Functions Premium, Logic Apps Standard all GA in Gov. **Container Apps: available in Azure Government** (FedRAMP audit-scope table; VA/TX region check pending tenant-side) — Track B's platform is viable for Gov. **Static Web Apps: not available in Gov** (use the App Service / Front Door + Storage fallback). **Azure Managed Redis: not available in Gov** (classic Azure Cache for Redis remains). **APIM v2 tiers: no US Gov regions in the official v2 region table** (assume classic tiers for any Gov APIM).
3. APIM tier — **MOOT for current state: no APIM exists in the commercial tenant** (Dec-2025's 3 instances gone/invisible). For any future APIM: classic Developer/Premium = VNet injection; Standard v2 = outbound integration + inbound PE; Premium v2 = full injection (GA, no classic→v2 migration); **v2 tiers list no US Gov regions** in the official region table (2026-09-07) — assume classic for Gov.
