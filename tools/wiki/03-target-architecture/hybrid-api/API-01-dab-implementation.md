...
```bash
# Create DAB configuration directory (external repo: k12-api-dab)
# Example: git clone <k12-api-dab> and work in that repo root
cd ../../k12-api-dab
...
External repo location (k12-api-dab): create `dab-config.json` at the repo root (not in this repo):
...
External repo location (k12-api-dab): add `Dockerfile` at the repo root (not in this repo):
...
External repo location (k12-api-dab): create `tests/dab-api.postman_collection.json` (not in this repo):
...
External repo location (k12-api-dab): create `tests/integration-tests.sh` (not in this repo):
